...
defmodule Imagem do
  ...
  def girar_90_horario(%Imagem{} = img) do
    ...
    %Imagem{img | matriz_pixeis: nova_matriz, largura: img.altura, altura: img.largura}
  end
  ...
  def aplicar_negativo(%Imagem{} = img) do
    nova_matriz =
      img.matriz_pixeis
      |> Enum.map(fn linha ->
        Enum.map(linha, fn {r, g, b} ->
          novo_r = img.cor_maxima - r
          novo_g = img.cor_maxima - g
          novo_b = img.cor_maxima - b

          {novo_r, node_g, novo_b} = {novo_r, novo_g, novo_b}
        end)
      end)
  ...
  def esquentar(%Imagem{} = img, tx) when tx in 0..100 do
    nova_matriz =
      ...
      |> Enum.map(fn linha ->
        Enum.map(linha, fn {r, g, b} ->
          novo_r = round(r + (img.cor_maxima - r) * tx / 100)
          novo_g = round(g + (img.cor_maxima - g) * tx / 100)
    ...
    nova_matriz =
      ...
      |> Enum.map(fn linha ->
        Enum.map(linha, fn {r, g, b} ->
          ...
          novo_b = round(b + (img.cor_maxima - b) * tx / 100)
...
# =========================================================================
# ATIVIDADE 3: Tratamento de Erro com `case`
# =========================================================================
IO.puts("\n...:: Testando a Atividade 3 ::...")

arquivo_para_ler = "op.ppm"
resultado_da_leitura = Imagem.ler(arquivo_para_ler)

case resultado_da_leitura do
  {:ok, imagem_pronta} ->
    IO.puts("SUCESSO: A imagem foi carregada perfeitamente!")
    IO.puts("-> Resolução: #{imagem_pronta.largura} x #{imagem_pronta.altura} píxeis.")
    IO.puts("-> Cor Máxima: #{imagem_pronta.cor_maxima}")

  {:error, motivo_do_erro} ->
    IO.puts("FALHA: Não foi possível carregar a imagem.")
    IO.puts("-> Motivo: #{motivo_do_erro}")
end